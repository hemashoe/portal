...
FILE_NAME = str(
    TPROGER_DIR + "tproger_data" + datetime.now().strftime("%m-%d_%H:%M") + ".csv"
)
    ...
    for data in data_parsed:
        ...
        if check_duplication is False:
            try:
                ...
                query = (
                    f"INSERT INTO main_post (title, slug, source_link, source_id, body, title_image, published, author_id)"
