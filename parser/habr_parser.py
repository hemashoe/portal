...
FILE_NAME = str(
    HABR_DIR + "habr_data" + datetime.now().strftime("%m-%d_%H:%M") + ".csv"
)
...
def get_article(posts_parsed: list) -> None:
    try:
        ...
        dataframe.to_csv(
            FILE_NAME, sep="'", header=True, index=True, index_label="post_id"
        )
    ...
    for data in data_parsed:
        ...
        if check_for_dublicate is False:
            try:
                ...
                query = (
                    f"INSERT INTO main_post (title, slug, source_link, source_id, description, body, title_image, published, author_id)"
                ...
                logger.error(
                    f"Unabe to update database, some error in {data['post_id']}"
                )
...
def main():
    logger.add(
        "../logs/habr_parser.log",
        format="{time} __|__ {level} __|__ {message}",
        level="INFO",
        rotation="100 MB",
        compression="zip",
    )
